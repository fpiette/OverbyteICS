...
TIcsFtpMulti is copyrighted software, but may be used freely.
...
7 Mar 2019 - V8.60 - Adapted for main ICS packages and FMX support.
  ...
  // alternate TLS and SSL
        ...
        begin
            ...
            // keep PWD response as root
                ...
                IcsPathDosToUnix (fServRootDir) ;  // 6 Jan 2006 change to Unix, 11 Sept 2008 was not changing directory!!
...
// create target directory
    ...
    if NOT ForceDirectories (newtardir) then
    begin
...
// build list of target files, so we don't copy unnecessary stuff
    ...
    if NOT ForceDirectories (basetardir) then
    begin
        result := TaskResFail ;
    ...
    begin
      ...
      // 21 Feb 2011 - if a directory, special handling
            ...
            // create target directory
                ...
                if NOT ForceDirectories (fnametar) then
...
// create archive directory
    ...
    begin
        fDelDone := false ;
        if NOT ForceDirectories (fUpArchDir) then
        begin
            inc (fCopyProg.ProcFailFiles) ;
            fCopyProg.ProcBytesDone := fCopyProg.ProcBytesLast + RFSize ;
...
var
    ...
    I, J, {donenr,} nodeltot, loop, retval, duration: integer ;
...
var
    Cancel: boolean ;
begin
    MagFTPClient := TIcsFtpMulti.Create (Nil) ;
    MagFTPClient.CopyEvent := LogEvent ;
    if FLogmaskName <> '' then
    begin
        FBuffLogStream := TIcsBuffLogStream.Create (MagFTPClient, FLogmaskName, '', FileCPUtf8) ;  // Format mask for log file name
        FIcsLog := TIcsLogger.Create (Nil) ;
        FIcsLog.OnIcsLogEvent := IcsLogEvent ;
        MagFTPClient.IcsLogger := FIcsLog ;
        FIcsLog.LogOptions := [] ;
    //    FIcsLog.LogOptions := [loDestEvent] + LogAllOptInfo ;
    end ;
    FAbort := false ;
    LogEvent (LogLevelInfo, 'FTP Thread Starting', Cancel) ;
    MagFTPClient.BulkMode := FBulkMode ;
    MagFTPClient.Multithreaded := true ;  // must use our own message pump
    MagFTPClient.LocalHost := FLocalHost ;
    MagFTPClient.HostName1 := FHostName1 ;
    MagFTPClient.HostName2 := FHostName2 ;
    MagFTPClient.UserName := FUserName ;
    MagFTPClient.Password := FPassword ;
    MagFTPClient.Port := FPort ;
    MagFTPClient.SocketFamily := FSocketFamily ; // March 2013
    MagFTPClient.SocketErrs := FSocketErrs ; // Nov 2016
    MagFTPClient.AttemptDelay := FAttemptDelay ;
    MagFTPClient.MaxAttempts := FMaxAttempts ;
    MagFTPClient.FailRepeat := FFailRepeat ;
    MagFTPClient.DataSocketSndBufSize := FDSocketSndBufSize ;
    MagFTPClient.DataSocketRcvBufSize := FDSocketRcvBufSize ;
    MagFTPClient.ConnectionType := FConnectionType ;
    MagFTPClient.SocksPort := FSocksPort ;
    MagFTPClient.SocksServer := FSocksServer ;
    MagFTPClient.ProxyPort := FProxyPort ;
    MagFTPClient.ProxyServer := FProxyServer ;
    MagFTPClient.SocksUsercode := FSocksUsercode ;
    MagFTPClient.SocksPassword := FSocksPassword ;
    MagFTPClient.Passive := FPassive ;
    MagFTPClient.HostType := FHostType ;
    MagFTPClient.XferMode := FXferMode ;
    MagFTPClient.CaseFile := FCaseFile ;
    MagFTPClient.DiffStampMins := FDiffStampMins ;
    MagFTPClient.Timeout := FTimeout ;
    MagFTPClient.DispLog := FDispLog ;
    MagFTPClient.DispFiles := FDispFiles ;
    MagFTPClient.DispRDir:= FDispRDir ;
    MagFTPClient.DispLDir:= FDispLDir ;
    MagFTPClient.UpArchDir := FUpArchDir ;
    MagFTPClient.UpArchive := FUpArchive ;
    MagFTPClient.ResFailed := FResFailed ;
    MagFTPClient.DispRemList := FDispRemList ;
    MagFTPClient.BulkMode := FBulkMode ;
    MagFTPClient.SrcDir := FSrcDir ;
    MagFTPClient.SrcFName := FSrcFName ;
    MagFTPClient.TarDir := FTarDir ;
    MagFTPClient.CopyType := FCopyType ;
    MagFTPClient.DelDone := FDelDone ;
    MagFTPClient.DelOldTar := FDelOldTar ;
    MagFTPClient.SubDirs := FSubDirs ;
    MagFTPClient.Mask := FMask ;
    MagFTPClient.Prev := FPrev ;
    MagFTPClient.Repl := FRepl ;
    MagFTPClient.ReplRO := FReplRO ;
    MagFTPClient.Safe := FSafe ;
    MagFTPClient.IgnoreFileExt := FIgnoreFileExt ;
    MagFTPClient.Options := FOptions ;
    MagFTPClient.BandwidthLimit := FBandwidthLimit ;
    MagFTPClient.MagFtpOpts := FMagFtpOpts ;
    MagFTPClient.NoProgress := FNoProgress ;
    MagFTPClient.EmptyDirs := fEmptyDirs ;
    MagFTPClient.SslSessCache := fSslSessCache ;          // 20 Apr 2015
    MagFTPClient.FtpSslPort := fFtpSslPort ;              // 20 Apr 2015
    MagFTPClient.FtpSslVerMethod := fFtpSslVerMethod ;    // 20 Apr 2015
    MagFTPClient.FtpSslRootFile := fFtpSslRootFile ;      // 20 Apr 2015
    MagFTPClient. FtpSslRevocation := fFtpSslRevocation ; // 20 Apr 2015
    MagFTPClient.FtpSslReportChain := fFtpSslReportChain ;// 20 Apr 2015
    FDirListing := '' ;
    case FtpThreadOpt of
        ftpthdList: FTaskRes := MagFTPClient.DispFtpDir (FDirListing) ;
        ftpthdDownCheck: FTaskRes := MagFTPClient.FtpDownload (true) ;
        ftpthdDownFiles: FTaskRes := MagFTPClient.FtpDownload (false) ;
        ftpthdUpCheck: FTaskRes := MagFTPClient.FtpUpload (true) ;
        ftpthdUpFiles: FTaskRes := MagFTPClient.FtpUpload (false) ;
    end ;
    FTotProcFiles := MagFTPClient.TotProcFiles ;
    FProcOKFiles := MagFTPClient.ProcOKFiles ;
    FDelOKFiles := MagFTPClient.DelOKFiles ;
    FProcFailFiles := MagFTPClient.ProcFailFiles ;
    FReqResponse := MagFTPClient.ReqResponse ;
    FSkippedFiles := MagFTPClient.SkippedFiles ;
    FInfo := 'FTP Thread Done, Task Result: ' + IcsGetTaskResName (FTaskRes) + ' - ' + MagFTPClient.ReqResponse + IcsCRLF ;
