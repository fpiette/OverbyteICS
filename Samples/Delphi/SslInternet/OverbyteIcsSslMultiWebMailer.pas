...
// see if page is being POSTed by itself to send and email
    ...
    begin
     ...
     // the IP check is to stop spammers POSTing this page without having GET it first, they have to know the IP and host name
        ...
        if (errorMsg = '') then
        begin
            if NOT WeblServerForm.IcsMailQueue.Active then WeblServerForm.StartQueueMail ;  // restart if not running
            if NOT WeblServerForm.IcsMailQueue.Active then
        ...
        begin
          // email it to hardcoded host
            try
                ...
                with WeblServerForm.IcsMailQueue.QuHtmlSmtp do begin
                    PlainText.Clear ;
                ...
                id := WeblServerForm.IcsMailQueue.QueueMail ;
                if id > 0 then begin
           // done OK
                ...
                begin
                    errorMsg := 'Failed to Queue Email Form: ' +
                                    WeblServerForm.IcsMailQueue.QuHtmlSmtp.ErrorMessage ;
                    Display (errorMsg) ;
                end ;
            except
