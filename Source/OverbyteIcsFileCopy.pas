...
TIcsFileCopy  HTTP Delphi component that allows indexing
...
procedure TIcsFileCopy.SetMultiDirList (value: string) ;
var
    I: integer ;
...
//        allowdiffDT := MinsToTime (DiffStamp) ;
        allowdiffDT := DiffStamp / (SecsPerDay / 60) ;
    IgnorePathList := TStringList.Create ;
    IgnorePathList.Delimiter := ';';
    IgnorePathList.DelimitedText := AnsiLowerCase (IgnorePaths) ;   // not sorted!!   // 16 May 2013
...
// create target directory
    ...
    if (Length (newdir) = 0) or (NOT ForceDirectories (newdir)) then
    begin
...
// make sure the destination is available
    if NOT ForceDirectories (CurTarDir) then
    begin
        result := TaskResFail ;
