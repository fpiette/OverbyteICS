...
type
    ...
    public
        ...
        function AddNameToCache(const URL, NewURL : string;
            DocType : ThtmlFileType; Error : boolean) : string;
...
begin
    ...
    if FileExists(Directory + CacheData) then begin
...
begin
    ...
    begin
        ...
        for I := 0 to URLList.Count - 1 do
            WriteLn(F, URLList[I], ' ', RedirectList[I], ' ', TypeList[I], ' ',
                FileList[I]);
        CloseFile(F);
    end;
    URLList.Free;
...
function TDiskCache.AddNameToCache(const URL, NewURL : string;
    DocType : ThtmlFileType; Error : boolean) : string;
var
    I : integer;
    S : string;
...
begin
    ...
    else begin { try searching in the Redirect list.  The back button would generate this }
...
begin
    while FileList.Count > MaxCacheEntries do begin
...
begin
    ...
    GoodExt := (Ext <> '') and ((Ext = '.htm') or (Ext = '.html') or
        (Ext = '.gif') or (Ext = '.jpg') or (Ext = '.png') or (Ext = '.bmp') or
        (Ext = '.txt') or (Ext = '.jpeg') or (Ext = '.css'));
